...
def create_advisor_agent():
    """Creates a new advisor agent with the latest environment variables"""
    provider = get_env_var('LLM_PROVIDER') or 'OpenAI'
    llm = get_env_var('PRIMARY_MODEL') or 'gpt-4o-mini'
    base_url = get_env_var('BASE_URL') or 'https://api.openai.com/v1'
    api_key = get_env_var('LLM_API_KEY') or 'no-llm-api-key-provided'
    
    print(f"DEBUG ADVISOR FACTORY - Using API key (first few chars): {api_key[:8]}...")
    print(f"DEBUG ADVISOR FACTORY - Provider: {provider}")
    print(f"DEBUG ADVISOR FACTORY - Base URL: {base_url}")
